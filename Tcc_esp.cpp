...
int main()
{
	...
	float tz2;
 	float ac = 0.01;
  	float dv = 0; // 0 a 2 
  	float d = 0.1;//0.1 a 1.5  
  	float fi1,fif,fi;
  	float fif2;
  	float fif4d,fif4t,fif4;
  	float fif3, fif3t,fif3d;
  	float fif1t,fi1d,fif1d,fif1;
  	float fif12t, fif12d;
...
//	  		d = 1.1;
			...
			s4 = d;	  		
	  		...
	  		sit = 0;
	  		float teste1 = dv+d2/2;
	  		float teste2 = z;
	  		float teste3 = x + h/3;
	  		
	  		if(((dv+d2/2) > z) && ((d <= x + h/3)))
	  			sit = 1;
	  		else if(((dv+d2/2) < z) && ((d > x + h/3)))	
	  			sit = 2;
			else if(((dv+d2/2) > z) && ((d > x + h/3)))	
	  			sit = 3;
			else if(((dv+d2/2) < z) && ((d <= x + h/3)))	
	  			sit = 4;
	  		else //if(sit != 1 && sit != 2 && sit != 3 && sit != 4)
				sit = 5;
			printf("SIT : %d\n",sit); 	  
			if(sit == 1)
			{
				...
				a = h11/s1;
				b = w11/s1;
				float bloco1 = (a/(pow(1+pow(a,2),0.5)));
				float bloco2 = (b/(pow(1+pow(a,2),0.5)));
				float bloco3 = (b/(pow(1+pow(b,2),0.5)));
				float bloco4 = (a/(pow(1+pow(b,2),0.5)));
...
//				float bloco4 = (a/(pow(1+pow(b,2),0.5)));
				fiz11 = (1/(2*M_PI))*( ((a/(pow(1+pow(a,2),0.5))) * (atan((b/(pow(1+pow(a,2),0.5))))) ) + ( (b/(pow(1+pow(b,2),0.5)))* (atan((a/(pow(1+pow(b,2),0.5))))) ) );
//				printf("1- s: %f\th: %f\tw: %f\ta: %f\tb: %f\n",s,h11,w11,a,b);
				
				h12 = dv;
				w12 = (w-r);
				a = h12/s;
				b = w12/s;
//				fiz12 = (1/(2*M_PI))*( (bloco1 * (atan(bloco2)) ) + ( bloco3* (atan(bloco4)) ) );
				fiz12 = (1/(2*M_PI))*( ((a/(pow(1+pow(a,2),0.5))) * (atan((b/(pow(1+pow(a,2),0.5))))) ) + ( (b/(pow(1+pow(b,2),0.5)))* (atan((a/(pow(1+pow(b,2),0.5))))) ) );
//				printf("12- s: %f\th: %f\tw: %f\ta: %f\tb: %f\n",s,h12,w12,a,b);
				fiz1s2 = fiz11 + fiz12;
...
//				printf("22- s: %f\th: %f\tw: %f\ta: %f\tb: %f\n",s,h22,w22,a,b);
				...
				fiz4s2 = fiz42 + fiz41;
				printf("fiz11: %f\tfiz12: %f\tfiz1s2: %f\n",fiz11,fiz12,fiz1s2);
				printf("fiz21: %f\tfiz22: %f\tfiz2s2: %f\n",fiz21,fiz22,fiz2s2);
				printf("fiz31: %f\tfiz32: %f\tfiz3s2: %f\n",fiz31,fiz32,fiz3s2);
				printf("fiz41: %f\tfiz42: %f\tfiz4s2: %f\n",fiz41,fiz42,fiz4s2);		
